...
def get_hotels(lat, lon):
    url = f"https://api.geoapify.com/v2/places?categories=accommodation.hotel&filter=circle:{lon},{lat},5000&limit=10&apiKey={API_KEY}"
    try:
        res = requests.get(url).json()
        return res.get("features", [])
    except requests.exceptions.RequestException as e:
        print(f"Error fetching hotels data: {e}")
        return []
